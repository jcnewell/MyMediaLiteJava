...
public class BPRMF extends MF { 
	...
	public double computeLoss() {
		...
		double complexity = 0;
		for (int c = 0; c < lossSampleU.length; c++) {
			complexity += regU * Math.pow(VectorUtils.euclideanNorm(userFactors.getRow(lossSampleU[c])), 2);
			complexity += regI * Math.pow(VectorUtils.euclideanNorm(itemFactors.getRow(lossSampleI[c])), 2);
			complexity += regJ * Math.pow(VectorUtils.euclideanNorm(itemFactors.getRow(lossSampleJ[c])), 2);
	...
	public double predict(int user_id, int item_id) {
		return itemBias[item_id] + MatrixUtils.rowScalarProduct(userFactors, user_id, itemFactors, item_id);
	}
