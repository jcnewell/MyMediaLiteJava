...
public class MatrixUtils {
   ...
   */
  public static void rowInitNormal(Matrix<Double> matrix, double mean, double stdev, int row) {
  ...
  public static void ColumnInitNormal(Matrix<Double> matrix, double mean, double stdev, int column) {
    Random random = Random.getInstance();
    for (int i = 0; i < matrix.dim1; i++) matrix.set(i, column, random.nextNormal(mean, stdev));
  }
  ...
  public static void inc(Matrix<Double> matrix1, Matrix<Double> matrix2) {
    if (matrix1.dim1 != matrix2.dim1 || matrix1.dim2 != matrix2.dim2) throw new IllegalArgumentException("Matrix sizes do not match.");
  ...
  public static double columnAverage(Matrix<Double> matrix, int col) {
    if (matrix.dim1 == 0) throw new IllegalArgumentException("Cannot compute average of 0 entries.");
    double sum = 0;
    for (int x = 0; x < matrix.dim1; x++) sum += (Double)matrix.data[x * matrix.dim2 + col];
  ...
  public static double rowAverage(Matrix<Double> matrix, int row) {
    if (matrix.dim2 == 0) throw new IllegalArgumentException("Cannot compute average of 0 entries.");
    double sum = 0;
    for (int y = 0; y < matrix.dim2; y++) sum += (Double)matrix.data[row * matrix.dim2 + y];
  ...
  public static double frobeniusNorm(Matrix<Double> matrix) {
    ...
    for (int x = 0; x < matrix.dim1 * matrix.dim2; x++) squared_entry_sum += Math.pow((Double)matrix.data[x], 2);
  ...
  public static double rowScalarProduct(Matrix<Double> matrix, int i, List<Double> vector) {
    if (i >= matrix.dim1) throw new IllegalArgumentException("i too big: " + i + ", dim1 is " + matrix.dim1);
    if (vector.size() != matrix.dim2) throw new IllegalArgumentException("wrong vector size: " + vector.size() + ", dim2 is " + matrix.dim2);
  ...
  public static Double rowScalarProduct(Matrix<Double> matrix1, int i, Matrix<Double> matrix2, int j) {
      if (i >= matrix1.dim1) throw new IllegalArgumentException("i too big: " + i + ", dim1 is " + matrix1.dim1);
      if (j >= matrix2.dim1) throw new IllegalArgumentException("j too big: " + j + ", dim1 is " + matrix2.dim1);
      if (matrix1.dim2 != matrix2.dim2) throw new IllegalArgumentException("wrong row size: " + matrix1.dim2 + " vs. " + matrix2.dim2);
  ...
  public static List<Double> rowDifference(Matrix<Double> matrix1, int i, Matrix<Double> matrix2, int j) {
    if (i >= matrix1.dim1) throw new IllegalArgumentException("i too big: " + i + ", dim1 is " + matrix1.dim1);
    if (j >= matrix2.dim1) throw new IllegalArgumentException("wrong row size: " + matrix1.dim2 + " vs. " + matrix2.dim2);
  ...
  public static Double rowScalarProductWithRowDifference(Matrix<Double> matrix1, int i, Matrix<Double> matrix2, int j, Matrix<Double> matrix3, int k) {
    if (i >= matrix1.dim1) throw new IllegalArgumentException("i too big: " + i + ", dim1 is " + matrix1.dim1);
    if (j >= matrix2.dim1) throw new IllegalArgumentException("j too big: " + j + ", dim1 is " + matrix2.dim1);
    if (j >= matrix3.dim1) throw new IllegalArgumentException("j too big: " + k + ", dim1 is " + matrix3.dim1);      
    if (matrix1.dim2 != matrix2.dim2) throw new IllegalArgumentException("wrong row size: " + matrix1.dim2 + " vs. " + matrix2.dim2);
    if (matrix1.dim2 != matrix3.dim2) throw new IllegalArgumentException("wrong row size: " + matrix1.dim2 + " vs. " + matrix3.dim2);
